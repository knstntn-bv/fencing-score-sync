...
`public_id` выдаётся на сервере: существующим профилям по `created_at` с 1001, новым — `max + 1`. Клиент его не задаёт и не меняет. В Account поле ID только для чтения. На чек-ине турнира по этому номеру можно добавить бойца (`lookup_checkin_by_public_id`); в протокол идёт постоянный `fencers.id`, даже если человека нет в ростере клуба-организатора.
...
Связать никнейм с аккаунтом: `link_fencer_to_profile`. Если person-строки ещё нет, `user_id` / `role` / имя / `public_id` пишутся на никнейм (его id становится постоянным). Если person-строка уже есть и без клуба — необратимое слияние: история (`matches`, `tournament_participants`, `tournament_bouts`) переписывается на person `id`, на person вешаются клуб и `role = member`, строка никнейма удаляется. Если человек уже в этом клубе или в другом, или никнейм и person встречаются в одном бое / турнире — отказ. Выйти из клуба: `unlink_and_archive` (с ростера) и `leave_own_club` (из Account) — обнуляют `club_id` и `role`, оставляют `id`, `user_id`, `public_id` и имя. Никнеймы по-прежнему архивируются на месте. Последнего owner снять нельзя. После выхода тот же id вешается на новый клуб через `create_own_club` / `add_linked_fencer`.

Имя связанной строки правит только владелец профиля (`save_own_profile`). Прямой UPDATE имени или архива на связанной строке отклоняется. В UI карандаш скрыт; свободную строку привязывают по ID; Archive для связанного человека снимает его с клуба, не архивирует.
