...
`public_id` выдаётся на сервере: существующим профилям по `created_at` с 1001, новым — `max + 1`. Клиент его не задаёт и не меняет. В Account поле ID только для чтения.
