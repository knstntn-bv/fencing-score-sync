...
Один клуб — несколько пользователей. Членство — активная строка `fencers` с `user_id` и `role` (`owner` / `trainer` / `member`). В каждом клубе хотя бы один `owner`. Клиент роли не показывает и клуб не переключает: после входа берётся связанный боец. Аккаунт без клуба видит табло и настройки, без ростера, истории и турниров.
...
fencers
  ...
  name          text not null
  public_id     text null unique       -- цифры; только у аккаунта, выдаёт сервер
  archived_at   timestamptz null       -- soft-delete: история не дырявится
  created_at    timestamptz
  updated_at    timestamptz
```

Никнейм (без аккаунта): `club_id` обязателен, `user_id` / `role` / `public_id` пустые. Человек с аккаунтом: `user_id` и `public_id` обязательны; без клуба `club_id` и `role` пустые, в клубе оба заданы. `public_id` ставит триггер при появлении person-строки (с 1001, потом max+1); клиент его не задаёт и не меняет. В Account поле ID только для чтения. На чек-ине турнира по этому номеру можно добавить бойца (`lookup_checkin_by_public_id`); в протокол идёт постоянный `fencers.id`, даже если человека нет в ростере клуба-организатора.

Уникальность имени: `(club_id, lower(trim(name)))` среди неархивных **в клубе**. Пустые имена запрещены. Непустой `user_id` уникален на всю таблицу. Доступ в клуб: активная строка с `user_id` текущего пользователя и непустым `club_id` (`is_club_member` / `is_club_owner`). Удаление клуба снимает `club_id` с человека и удаляет никнеймы.

`save_own_name` создаёт или обновляет person-строку (без клуба, если клуба ещё нет). `create_own_club` и `add_linked_fencer` вешают клуб на уже существующую строку, не вставляют второго бойца.

Связать никнейм с аккаунтом: `merge_nickname_into_person` — необратимое слияние: история (`matches`, `tournament_participants`, `tournament_bouts`) переписывается на person `id`, на person вешаются клуб и `role = member`, строка никнейма удаляется. Если человека с таким `public_id` нет, он уже в этом клубе или в другом, или никнейм и person встречаются в одном бое / турнире — отказ. Выйти из клуба: `detach_from_club` (с ростера) и `leave_own_club` (из Account) — обнуляют `club_id` и `role`, оставляют `id`, `user_id`, `public_id` и имя. Никнеймы по-прежнему архивируются на месте. Последнего owner снять нельзя. После выхода тот же id вешается на новый клуб через `create_own_club` / `add_linked_fencer`.

Имя связанной строки правит только владелец аккаунта (`save_own_name`). Прямой UPDATE имени или архива на связанной строке отклоняется. В UI карандаш скрыт; свободную строку привязывают по ID; Archive для связанного человека снимает его с клуба, не архивирует.

Удаление никнейма из UI = архив. Слияние никнейма с аккаунтом удаляет строку через RPC. На `matches` нет каскадного удаления бойцов. Клиентской политики DELETE нет.
...
Регистрация больше не создаёт клуб. После входа, если имени ещё нет, экран: имя (обязательно) и опции **Create club** (имя клуба) или **Skip**. Имя пишется в `fencers` через `save_own_name()`; `public_id` ставит триггер. Клуб — `create_own_club()`: строка в `clubs`, существующая person-строка получает `club_id` и `role = owner`. Старые аккаунты уже в клубах; в Account у них имя и неизменяемый ID. Можно выйти из клуба и создать другой — id тот же.
...
После сессии клиент читает свою person-строку в `fencers` (`user_id`). Если её нет, имени ещё нет (онбординг). Если `club_id` пустой, клубных страниц нет. Списки дополнительно фильтруются по `club_id`.

Политики: `is_club_member(club_id)` смотрит на активного бойца с `user_id = auth.uid()`, не `club_id = auth.uid()`. Сейчас все роли клуба имеют одинаковый доступ к ростеру и записи боёв. Владелец может UPDATE `clubs.name` (`is_club_owner`). `save_own_name()` обновляет имя на person-строке. Клиентский insert в `fencers` не ставит `user_id`; update не меняет `user_id` и `role`, и не меняет имя/архив у связанной строки. Слияние никнейма и снятие с клуба — только RPC. Anon таблицы не читает и не пишет. `matches` только INSERT+SELECT, без UPDATE/DELETE.
...
2. **`/fencers`** — список, добавление имени или **Add by ID**, переименование свободных имён, архив / restore. Свободную строку привязывают к аккаунту (иконка цепочки): никнейм заменяется person-строкой, история переезжает на id аккаунта, обратно нельзя. Связанный боец: имя только из Account, Archive = снять с клуба (id остаётся). Без клуба маршрут закрыт.
...
7. **Account** `/settings/account` — переименовать себя (и строку в ростере); владелец меняет имя клуба; выйти из клуба (id остаётся); без клуба — создать клуб; Sign out.
