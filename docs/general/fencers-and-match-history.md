...
Связать никнейм с аккаунтом, у которого ещё нет строки `fencers`: `link_fencer_to_profile`. Если person-строка уже есть, связь отклоняется (слияние — следующий этап). Выйти из клуба: `unlink_and_archive` (с ростера) и `leave_own_club` (из Account) — обнуляют `club_id` и `role`, оставляют `id`, `user_id`, `public_id` и имя. Никнеймы по-прежнему архивируются на месте. Последнего owner снять нельзя. После выхода тот же id вешается на новый клуб через `create_own_club` / `add_linked_fencer`.
...
2. **`/fencers`** — список, добавление имени или **Add by ID**, переименование свободных имён, архив / restore. Свободную строку можно привязать к аккаунту (иконка цепочки, номер из Account). Связанный боец: имя только из Account, Archive = снять с клуба (id остаётся). Без клуба маршрут закрыт.
