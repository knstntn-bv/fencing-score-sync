...
Уникальность имени в клубе: `(club_id, lower(trim(name)))` среди неархивных. Пустые имена запрещены. `role` и `user_id` либо оба пустые, либо оба заданы. Непустой `user_id` уникален на всю таблицу (один профиль — один клуб, в том числе среди архива). Доступ в клуб: активная строка с `user_id` текущего пользователя (`is_club_member` / `is_club_owner`). `club_members` ещё пишется при создании клуба, клиент его больше не читает.
