...
Политики: `is_club_member(club_id)`, не `club_id = auth.uid()`. Сейчас все роли клуба имеют одинаковый доступ к ростеру и записи боёв. `club_members` с клиента только SELECT (свои членства). Владелец может UPDATE `clubs.name` (`is_club_owner`). `save_own_profile()` обновляет и связанную строку в `fencers`. Клиентский insert в `fencers` не ставит `user_id` (это делает `create_own_club`). Anon таблицы не читает и не пишет. `matches` только INSERT+SELECT, без UPDATE/DELETE.
