...
Один клуб — несколько пользователей. В каждом клубе хотя бы один `owner`. Клиент роли не показывает и клуб не переключает: после входа берётся членство owner, иначе самое раннее. Аккаунт без клуба видит только настройки.
...
Регистрация больше не создаёт клуб. После входа, если имени ещё нет, экран: имя (обязательно) и опции **Create club** (имя клуба) или **Skip**. Имя пишется в `profiles` через `save_own_profile()`. Клуб — `create_own_club()`: строка в `clubs`, членство `owner`, строка в `fencers` с именем профиля и `user_id`. Старые аккаунты уже в клубах; им показывают только поле имени.

Без клуба закрыты ростер, история, турниры и табло: доступны только `/settings`. Клуб можно создать оттуда позже.

После сессии клиент читает `profiles` и `club_members`. Если членства нет, `club_id` остаётся пустым. Списки дополнительно фильтруются по `club_id`.

Политики: `is_club_member(club_id)`, не `club_id = auth.uid()`. Сейчас все роли клуба имеют одинаковый доступ к ростеру и записи боёв. `clubs` / `club_members` с клиента только SELECT (свои членства). Клиентский insert в `fencers` не ставит `user_id` (это делает `create_own_club`). Anon таблицы не читает и не пишет. `matches` только INSERT+SELECT, без UPDATE/DELETE.
...
2. **`/fencers`** — список, добавление, переименование, архив / restore, переключатель архивных, кнопка индивидуальной статистики. Не смешивается с лимитами в Settings. Без клуба маршрут закрыт.
...
6. **Settings** — лимиты на устройстве; для вошедшего — имя, выход; без клуба — форма создания клуба; со клубом — ссылки на ростер/историю/турниры.

Навигация с табло: иконки Users / Tournaments / History / Settings. У гостя вместо клубных — Sign in. У вошедшего без клуба клубных иконок нет, открываются только настройки.
...
- UI нескольких клубов, приглашений и разных прав ролей (в схеме `owner` / `trainer` / `member` уже есть; владелец клуба попадает в ростер).
