...
Связать строку ростера с аккаунтом: `link_fencer_to_profile(fencer_id, public_id)` — ставит `user_id`, `role = member` и копирует имя из профиля (`fencing.fencer_link = 1`). Отвязать: `unlink_and_archive(fencer_id)` — `user_id` и `role` в null, архив, удаление из `club_members`. Последнего owner отвязать нельзя.

Имя связанной строки правит только владелец профиля (`save_own_profile`). Прямой UPDATE имени или архива на связанной строке отклоняется. В UI карандаш скрыт; Archive для связанного человека идёт через unlink.
...
2. **`/fencers`** — список, добавление, переименование свободных имён, архив / restore, переключатель архивных, кнопка индивидуальной статистики. Связанный с аккаунтом боец: имя только из Account, Archive = unlink. Без клуба маршрут закрыт.
