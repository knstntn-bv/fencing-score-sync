...
Уникальность имени в клубе: `(club_id, lower(trim(name)))` среди неархивных. Пустые имена запрещены. `role` и `user_id` либо оба пустые, либо оба заданы. Непустой `user_id` уникален на всю таблицу (один профиль — один клуб, в том числе среди архива). Пока доступ в клуб ещё проверяется через `club_members`; роль на бойце заполняется с членства и пишется при `create_own_club`.
