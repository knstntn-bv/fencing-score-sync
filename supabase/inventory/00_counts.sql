...
from (
  ...
  union all select 'A2_users_without_profile', 'info',
    (
  ...
  union all select 'B_linked_without_profile', 'review',
    (
      select count(*) from linked f
      where not exists (
  ...
  union all select 'B2_profile_multiple_fencers', 'review',
    (
      select count(*) from (
        select user_id from public.fencers
  ...
  union all select 'C_archived_still_linked', 'review',
  ...
  union all select 'E_guest_profile', 'rewrite',
    (
      select count(*) from guest_kind
      where is_guest = true and id_kind = 'profile_user'
    ),
    'guest fencer_id = profiles.user_id; rewrite to person fencers.id'
  union all select 'E_guest_is_fencer', 'review',
    (
      select count(*) from guest_kind
      where is_guest = true and id_kind = 'fencer_row'
    ),
    'is_guest but id is a fencers.id; policy mismatch'
  union all select 'E_guest_neither', 'review',
    (
      select count(*) from guest_kind
      where is_guest = true and id_kind = 'neither'
    ),
    'guest id is neither profiles.user_id nor fencers.id; orphan'
  union all select 'E_roster_missing_fencer', 'review',
    (
  ...
  union all select 'E_roster_marked_wrong', 'review',
    (
      select count(*) from guest_kind
      where is_guest = false and id_kind <> 'fencer_row'
    ),
    'is_guest = false but id is not a fencers.id'
  union all select 'E_rewrite_collision', 'review',
    (
      select count(*)
  ...
  union all select 'F_same_club_name', 'review',
    (
      select count(*)
      from nicknames n
  ...
  union all select 'F_name_like_profile', 'info',
    (
  ...
  union all select 'F_merge_same_match', 'review',
    (
      select count(*)
  ...
  union all select 'F_merge_same_bout', 'review',
    (
      select count(*)
  ...
  union all select 'G_match_club_mismatch', 'review',
    (
  ...
  union all select 'G_bout_id_not_participant', 'review',
    (
      select count(*) from public.tournament_bouts b
      where (
  ...
  union all select 'H_linked_name_mismatch', 'review',
    (
      select count(*)
      from linked f
    ...
    'active linked fencers.name differs from profiles.name'
  union all select 'J_club_without_active_owner', 'review',
    (
  ...
  union all select 'public_id_max', 'info',
    (select coalesce(max(public_id::bigint), 0) from public.profiles),
    'max profiles.public_id (person rows will keep these digits)'
) as src(check_id, kind, n, meaning)
order by
  case kind
    when 'review' then 0
    when 'rewrite' then 1
